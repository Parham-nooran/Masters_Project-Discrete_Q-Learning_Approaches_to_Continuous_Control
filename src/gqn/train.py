...
class GQNTrainer(Logger):
    ...
    def train(self):
        ...
        obs_buffer = OptimizedObsBuffer(obs_shape, self.device)
        ...
        for episode in range(start_episode, self.config.num_episodes):
            ...
            if episode % self.config.detailed_log_interval == 0 and episode > 0:
                ...
                self.logger.info(f"Original Reward: {original_episode_reward:.2f}")
                self.logger.info(
...
def create_gqn_config(args):
    ...
    # GQN-specific overrides
    config.action_penalty = 0.001
