...
def _add_action_space_arguments(parser):
    ...
    parser.add_argument(
        "--initial-bins", type=int, default=2, help="Initial number of bins"
    )
    parser.add_argument(
        "--final-bins", type=int, default=9, help="Final number of bins"
    )
    parser.add_argument(
        "--growing-schedule",
        type=str,
...
def _add_exploration_arguments(parser):
    ...
    parser.add_argument(
        "--epsilon", type=float, default=0.1, help="Initial epsilon for exploration"
    )
    parser.add_argument(
        "--epsilon-decay", type=float, default=0.995, help="Epsilon decay rate"
    )
    parser.add_argument(
        "--min-epsilon", type=float, default=0.01, help="Minimum epsilon"
    )
...
def _add_optimization_arguments(parser):
    ...
    parser.add_argument(
        "--huber-delta", type=float, default=1.0, help="Huber loss delta"
    )
    parser.add_argument(
        "--gradient-clip", type=float, default=40.0, help="Gradient clipping value"
    )
    parser.add_argument(
        "--action-penalty-coeff",
        type=float,
        default=0.0,
...
def _add_replay_buffer_arguments(parser):
    ...
    parser.add_argument(
        "--per-alpha", type=float, default=0.6, help="PER alpha parameter"
    )
    parser.add_argument(
        "--per-beta", type=float, default=0.4, help="PER beta parameter"
    )


def _add_network_arguments(parser):
    """Add neural network architecture arguments."""
    parser.add_argument("--layer-size", type=int, default=512, help="Hidden layer size")
    parser.add_argument(
        "--layer-size-bottleneck", type=int, default=50, help="Encoder bottleneck size"
    )
    parser.add_argument(
