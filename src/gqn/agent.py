...
class GQNAgent:
    ...
    def __init__(self, config, obs_shape, action_spec):
        ...
        self.obs_dim = obs_shape[0] if len(obs_shape) == 1 else config.layer_size_bottleneck if hasattr(config, 'layer_size_bottleneck') else 50
    ...
    def update(self):
        ...
        metrics = self._compute_loss(obs_encoded, actions, rewards, next_obs_encoded, 
    ...
    def _compute_loss(self, obs, actions, rewards, next_obs, dones, discounts, weights, indices):
        ...
        return {
            ...
            'mse_loss1': loss1.mean().item(),
            'mse_loss2': loss2.mean().item()
        }
