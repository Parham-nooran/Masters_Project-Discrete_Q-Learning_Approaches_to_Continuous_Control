...
class PlottingUtils:
    ...
    def plot_success_rate(self, window=100, save=False, save_dir=None):
        ...
        successes = np.array(
            [s if s is not None else 0.0 for s in self.metrics.episode_successes]
        )
    ...
    def _print_success_stats(self):
        ...
        if len(successes) > 0:
            ...
            if len(successes) >= 100:
                ...
                recent_success_rate = (
                    np.sum(recent_successes) / len(recent_successes)
                ) * 100
                self.logger.info(
                    f"Recent 100 episodes success rate: {recent_success_rate:.2f}%"
                )
